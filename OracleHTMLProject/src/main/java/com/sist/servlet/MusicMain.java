...
public class MusicMain extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 *  	=> web.xml에 등록된 데이터 읽기
		 *  	=> 자동 로그인 (Cookie...)
		 *  2. service() : 사용자가 요청 시마다 호출
		 *  	= doGet()
		 *  	= doPost()
		 *  3. destory() : 새로고침 / 화면 이동 => 자동 메모리 회수
		 *  	=> 호출 시마다 초기화
		 *  	=> 단점 : 이전 페이지로 이동 X => 첫 페이지로 이동
		 *  			  Ajax => React => Vue 
		 *  
		 * MVC => Back / Front
		 ...
		 *   1. 브라우저에서 서버 연결
		 *   2. 데이터 전송
		 *   3. Servlet을 받는다 => doGet/dePost
		 *   4. 처리 후 HTML을 브라우저로 전송
		 *   5. 브라우저 화면 출력
		 *   
		 *   out.println("<html>")
		 ...
		 * 54page
		 *  HTML 구조
		 *  <!DOCTYPE html> => html5버전을 읽어온다
		 *  <html> => 문서의 시작
		 *  	<head>
		 *  		화면 출력 X
		 *  		설정 파일
		 *  		CSS / JavaScript 파일 읽기 => 외부 (파일마다 공통)
		 *  		=> 내부 (파일 안에서만 사용)
		 *  		CSS 설정
		 *  		JavaScript 설정
		 *  	</head>
